...
var n = cp.fork(__dirname + '/subProcess.js');
    ...
    socket.on('data', (dataRaw) => {
        ...
        if (
            ...
            if (!sockets[socket.channel]) {
                servers[socket.channel] = cp.fork(__dirname + '/subProcess.js');
                servers[socket.channel].send({
                    starting: true,
...
function _destroySocket(socket) {
    ...
    if (Object.keys(sockets[socket.channel]).length === 1) {
        ...
        servers[socket.channel].kill('SIGINT');
        delete servers[socket.channel];
        console.log("deleted server channel: " + socket.channel);
    }
}
